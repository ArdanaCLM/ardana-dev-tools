    ...
    refspec:
        ...
        description:
            - Add an additional refspec to be fetched.
              ...
              or tag, this option may be necessary to specify the ref
              containing the I(SHA-1).
    ...
    depth:
        ...
        description:
            - Create a shallow clone with a history truncated to the specified
              number or revisions. The minimum possible value is C(1),
              otherwise ignored.
    clone:
        required: false
        default: "yes"
        ...
        version_added: "1.9"
        description:
            - If C(no), do not clone the repository if it does not exist
              locally
    update:
        required: false
        default: "yes"
        ...
        version_added: "1.2"
        description:
            - If C(no), do not retrieve new revisions from the origin
              repository
    executable:
        required: false
        default: null
...
notes:
    - "If the task seems to be hanging, first verify remote host is in
      C(known_hosts). SSH will prompt user to authorize the first contact with
      a remote host.  To avoid this prompt, one solution is to add the remote
      host public key in C(/etc/ssh/ssh_known_hosts) before calling the git
...
- git: repo=https://github.com/ansible/ansible-examples.git
       dest=/src/ansible-examples refspec=+refs/pull/*:refs/heads/*
'''
...
def get_submodule_versions(git_path, module, dest, version='HEAD'):
    ...
    for line in out.splitlines():
        if line.startswith("Entering '"):
            ...
            module.fail_json(msg='Unable to parse submodule hash line: %s' %
                             line.strip())
    if subm_name is not None:
        module.fail_json(msg='Unable to find hash for submodule: %s' %
                         subm_name)
...
def clone(git_path, module, repo, dest, remote, depth, version, bare,
    ...
    if bare:
        if remote != 'origin':
            module.run_command([git_path, 'remote', 'add', remote, repo],
                               check_rc=True, cwd=dest)

    if refspec:
        module.run_command([git_path, 'fetch', remote, refspec],
                           check_rc=True, cwd=dest)
    ...
    if version == 'HEAD':
        if cloning:
            ...
            head_branch = get_head_branch(git_path, module, dest, remote,
                                          bare)
            cmd = '%s ls-remote %s -h refs/heads/%s' % (git_path, remote,
                                                        head_branch)
...
def is_remote_branch_or_tag(git_path, module, dest, remote, version):
    cmd = ('%(git_path)s ls-remote %(remote)s -h refs/heads/%(version)s '
           '-t refs/tags/%(version)s' % (
               {'git_path': git_path, 'remote': remote, 'version': version}))
    (rc, out, err) = module.run_command(cmd, check_rc=True, cwd=dest)
    if version in out:
        return True
    else:
...
def get_branches(git_path, module, dest):
    ...
    (rc, out, err) = module.run_command(cmd, cwd=dest)
    if rc != 0:
        module.fail_json(msg="Could not determine branch data - received %s" %
                         out)
...
def get_tags(git_path, module, dest):
    ...
    (rc, out, err) = module.run_command(cmd, cwd=dest)
    if rc != 0:
        module.fail_json(msg="Could not determine tag data - received %s" %
                         out)
    for line in out.split('\n'):
...
def get_head_branch(git_path, module, dest, remote, bare=False):
    ...
    This is partly taken from lib/ansible/utils/__init__.py.  It finds the
    correct path to .git/HEAD and reads from that file the branch that HEAD is
    associated with.  In the case of a detached HEAD, this will look up the
    branch in .git/refs/remotes/<remote>/HEAD.
    '''
    if bare:
        repo_path = dest
    else:
        repo_path = os.path.join(dest, '.git')
    # Check if the .git is a file. If it is a file, it means that we are in a
    # submodule structure.
...
def fetch(git_path, module, repo, dest, version, remote, bare, refspec):
    ...
    commands = [("set a new url %s for %s" % (repo, remote),
                 [git_path, 'remote', 'set-url', remote, repo])]
...
def submodules_fetch(git_path, module, remote, track_submodules, dest):
    ...
    if not changed:
        ...
        if track_submodules:
            ...
            (rc, out, err) = module.run_command(cmd, check_rc=True, cwd=dest)
            if rc != 0:
                module.fail_json(msg='Failed to retrieve submodule status: %s'
                                 % out + err)
            for line in out.splitlines():
                if line[0] != ' ':
                    changed = True
...
def submodule_update(git_path, module, dest, track_submodules):
    ...
    (rc, out, err) = module.run_command(cmd, cwd=dest)
    if rc != 0:
        module.fail_json(msg="Failed to init/update submodules: %s" %
                         out + err)
    return (rc, out, err)
...
def switch_version(git_path, module, dest, remote, version):
    ...
    if version != 'HEAD':
        ...
        (rc, out, err) = module.run_command("%s checkout --force %s" %
                                            (git_path, branch), cwd=dest)
        if rc != 0:
...
def main():
    ...
    if module.params['ssh_opts'] is not None:
        ...
        add_git_host_key(
            module, repo, accept_hostkey=module.params['accept_hostkey'])
    ...
    local_mods = False
    repo_updated = None
    if ((dest and not os.path.exists(gitconfig)) or
            (not dest and not allow_clone)):
        ...
        if before == remote_head:
            if local_mods:
                module.exit_json(changed=True, before=before,
                                 after=remote_head,
                                 msg="Local modifications exist")
            elif is_remote_branch_or_tag(git_path, module, dest, repo,
                                         version):
                # if the remote is a branch/tag and we have the it locally,
                # exit early
                if version in get_tags(git_path, module, dest):
                    repo_updated = False
                if is_local_branch(git_path, module, dest, version):
                    repo_updated = False
            else:
                repo_updated = False
    ...
    if recursive and not bare:
        ...
        if module.check_mode:
            if submodules_updated:
                module.exit_json(changed=True, before=before,
                                 after=remote_head, submodules_changed=True)
            else:
                module.exit_json(changed=False, before=before,
                                 after=remote_head)
...
from ansible.module_utils.known_hosts import *  # noqa


class MyAnsibleModule(AnsibleModule):
    """Overload AnsibleModule's run_command method with retry version"""

    def run_command(self, *args, **kwargs):
        retries = 3
        while retries > 0:
            (rc, out, err) = super(MyAnsibleModule,
                                   self).run_command(*args, **kwargs)
            retries -= 1
            # if we succeeded or have exceeded retries
            if rc == 0:
                # worked
                return (rc, out, err)

            if retries <= 0:
                # failed, ad a bit more info
                err += ", after %s retries" % (3 - retries)
                return (rc, out, err)
